...
import { customerRowSchema, type CustomerRowInput } from "@/lib/validation/customers";
...
type ResolvedCustomerRow = CustomerRowInput & { groupIds: string[] };
...
export async function bulkUploadCustomersAction(
  _prev: BulkUploadState,
  formData: FormData,
): Promise<BulkUploadState> {
  const context = await requireDashboardContext();
  requirePermission(context.membership, "customers", "create");
  ...
  const result = await runBulkImport<CustomerRowInput, ResolvedCustomerRow>({
    rows: parsedCsv.rows,
    rowSchema: customerRowSchema,
    resolveRow: async (data) => {
      const names = splitGroupNames(data.groupNames);
      const groups = await Promise.all(
        names.map((name) => findOrCreatePartyGroupByName(context.activeBusinessId, "customer", name)),
      );
  ...
  return {
    success: `Imported ${result.insertedCount} of ${result.totalRows} customer(s)${
