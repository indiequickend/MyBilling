...
import { bulkUploadCustomersAction } from "./actions";

export default async function CustomersBulkUploadPage() {
  const context = await getDashboardContext();
  if (!context) redirect("/login");
  if (!context.activeBusinessId || !context.membership) redirect("/");

  if (!can(context.membership, "customers", "create")) {
    return <p className="text-sm text-destructive">You don&apos;t have permission to import customers.</p>;
  }

  return (
    <div>
      <h1 className="mb-2 text-lg font-semibold">Bulk upload customers</h1>
      <p className="mb-6 max-w-2xl text-sm text-muted-foreground">
        Upload a CSV with one customer per row. Up to {BULK_IMPORT_MAX_ROWS} rows per file. Valid rows
      ...
      <BulkUploadForm action={bulkUploadCustomersAction} />
    </div>
  );
}
