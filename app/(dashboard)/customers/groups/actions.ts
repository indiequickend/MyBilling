...
export async function createCustomerGroupAction(
  ...
  const parsed = partyGroupSchema.safeParse({ type: "customer", name: formData.get("name") });
  if (!parsed.success) {
...
export async function updateCustomerGroupAction(formData: FormData): Promise<void> {
  const context = await requireGroupsPermission();
...
export async function softDeleteCustomerGroupAction(formData: FormData): Promise<void> {
  const context = await requireGroupsPermission();
  const groupId = String(formData.get("groupId") ?? "");
  if (!groupId) return;
...
export async function restoreCustomerGroupAction(formData: FormData): Promise<void> {
  const context = await requireGroupsPermission();
  const groupId = String(formData.get("groupId") ?? "");
  if (!groupId) return;
