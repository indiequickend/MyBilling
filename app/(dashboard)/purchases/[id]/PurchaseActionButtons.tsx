...
import { cancelPurchaseAction, softDeletePurchaseAction, type PurchaseActionState } from "../actions";
...
export function CancelPurchaseButton({ purchaseId }: { purchaseId: string }) {
  const [state, formAction] = useActionState(cancelPurchaseAction, initialState);
  return (
...
export function DeletePurchaseButton({ purchaseId }: { purchaseId: string }) {
  const [state, formAction] = useActionState(softDeletePurchaseAction, initialState);
  return (
    <form action={formAction} className="inline-flex flex-col items-end gap-1">
      <input type="hidden" name="purchaseId" value={purchaseId} />
