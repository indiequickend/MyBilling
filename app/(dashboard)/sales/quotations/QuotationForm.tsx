...
export type QuotationFormDefaultValues = {
  customerId: string;
...
export function QuotationForm({
  ...
  const [state, formAction] = useActionState(saveQuotationAction, initialState);

  const canDraft = mode === "create" || editableStatus === "draft";
  const placeOfSupplyState = defaultValues?.placeOfSupplyState ?? businessState;

  // Mirrors the Discount/round-off fields below so the line items totals summary stays live as
  // the user types, without turning those fields into fully controlled inputs.
          ...
          <FieldGroup>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <FormField
                ...
                type="date"
                required
                defaultValue={defaultValues?.quotationDate ?? new Date().toISOString().slice(0, 10)}
                error={state.fieldErrors?.quotationDate}
              />
              <FormField
                label="Valid until"
                name="validUntil"
