 ...
 * so this runs on Vercel serverless functions without a Chromium binary. Pro Forma Invoices are
 * standalone (no conversion action) but otherwise reuse the Invoice PDF shape minus the
 * paid/balance-due rows, since there's no payment tracking on this document type. */
export async function ProformaInvoiceDocument(data: ProformaInvoiceTemplateData) {
  const { proformaInvoice, business, bankAccount, signature } = data;
  ...
  if (bankAccount?.upiId) {
    ...
    )}&am=${(proformaInvoice.grandTotalMinor / 100).toFixed(2)}&cu=INR`;
    qrDataUri = await QRCode.toDataURL(upiUrl, { margin: 1, width: 240 });
  }

  const billingAddress = addressLine(business.addresses?.billing);
  const customerAddress = addressLine(proformaInvoice.customerSnapshot.billingAddress);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.alignRight}>
            ...
            {proformaInvoice.referenceNumber ? (
              <Text style={styles.muted}>Ref: {proformaInvoice.referenceNumber}</Text>
            ) : null}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text>{proformaInvoice.customerSnapshot.displayName}</Text>
          {proformaInvoice.customerSnapshot.gstin ? (
            <Text>GSTIN: {proformaInvoice.customerSnapshot.gstin}</Text>
          ) : null}
        ...
        <View style={styles.table}>
          ...
          {proformaInvoice.lineItems.map((li, idx) => (
        ...
        <View style={styles.totals}>
          <View style={styles.totalsRow}>
            <Text>Subtotal</Text>
            <Text>Rs. {minorToRupeesString(proformaInvoice.subtotalMinor)}</Text>
          </View>
          <View style={styles.totalsRow}>
            <Text>Tax (CGST+SGST/IGST)</Text>
            <Text>Rs. {minorToRupeesString(proformaInvoice.totalTaxMinor)}</Text>
          </View>
          {proformaInvoice.discountAmountMinor > 0 ? (
            <View style={styles.totalsRow}>
              <Text>Discount</Text>
              <Text>-Rs. {minorToRupeesString(proformaInvoice.discountAmountMinor)}</Text>
            </View>
          ) : null}
          {proformaInvoice.roundOff && proformaInvoice.roundOffAmountMinor !== 0 ? (
            <View style={styles.totalsRow}>
              <Text>Round off</Text>
              <Text>Rs. {minorToRupeesString(proformaInvoice.roundOffAmountMinor)}</Text>
            </View>
          ) : null}
          <View style={styles.grandRow}>
            <Text style={styles.bold}>Grand Total</Text>
            <Text style={styles.bold}>Rs. {minorToRupeesString(proformaInvoice.grandTotalMinor)}</Text>
          </View>
        </View>
