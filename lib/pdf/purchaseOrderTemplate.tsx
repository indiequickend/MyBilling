 ...
 * payment/bank-account concerns (a PO is a pre-purchase commitment, not a billed document). */
export async function PurchaseOrderDocument(data: PurchaseOrderTemplateData) {
  const { purchaseOrder, business } = data;

  const billingAddress = addressLine(business.addresses?.billing);
  const vendorAddress = addressLine(purchaseOrder.vendorSnapshot.billingAddress);
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.alignRight}>
            <Text style={styles.bold}>Purchase Order {purchaseOrder.docNumber ?? "(draft)"}</Text>
            <Text style={styles.muted}>Date: {formatDate(purchaseOrder.orderDate)}</Text>
            {purchaseOrder.expectedDeliveryDate ? (
              <Text style={styles.muted}>Expected: {formatDate(purchaseOrder.expectedDeliveryDate)}</Text>
            ) : null}
            {purchaseOrder.referenceNumber ? (
              <Text style={styles.muted}>Ref: {purchaseOrder.referenceNumber}</Text>
            ) : null}
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text>{purchaseOrder.vendorSnapshot.displayName}</Text>
          {purchaseOrder.vendorSnapshot.gstin ? (
            <Text>GSTIN: {purchaseOrder.vendorSnapshot.gstin}</Text>
          ) : null}
        ...
        <View style={styles.totals}>
          <View style={styles.totalsRow}>
            <Text>Subtotal</Text>
            <Text>Rs. {minorToRupeesString(purchaseOrder.subtotalMinor)}</Text>
          </View>
          <View style={styles.totalsRow}>
            <Text>Tax (CGST+SGST/IGST)</Text>
            <Text>Rs. {minorToRupeesString(purchaseOrder.totalTaxMinor)}</Text>
          </View>
          {purchaseOrder.discountAmountMinor > 0 ? (
            <View style={styles.totalsRow}>
              <Text>Discount</Text>
              <Text>-Rs. {minorToRupeesString(purchaseOrder.discountAmountMinor)}</Text>
            </View>
          ) : null}
          {purchaseOrder.roundOff && purchaseOrder.roundOffAmountMinor !== 0 ? (
            <View style={styles.totalsRow}>
              <Text>Round off</Text>
              <Text>Rs. {minorToRupeesString(purchaseOrder.roundOffAmountMinor)}</Text>
            </View>
          ) : null}
          <View style={styles.grandRow}>
            <Text style={styles.bold}>Grand Total</Text>
            <Text style={styles.bold}>Rs. {minorToRupeesString(purchaseOrder.grandTotalMinor)}</Text>
          </View>
        </View>
