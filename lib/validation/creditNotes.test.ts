...
import { creditNoteLineItemSchema, creditNoteDiscountSchema } from "@/lib/validation/creditNotes";
...
describe("creditNoteLineItemSchema", () => {
  it("normalizes a percentage discount to a raw 0-100 number", () => {
    const result = creditNoteLineItemSchema.safeParse({
      ...baseLineItem,
...
describe("creditNoteDiscountSchema", () => {
  it("parses a document-level percentage discount with its target", () => {
    const result = creditNoteDiscountSchema.safeParse({
      discountType: "percentage",
      discountValue: "5",
