...
const rawQuotationLineItemSchema = z.object({
...
export const quotationLineItemSchema = rawQuotationLineItemSchema.transform((val, ctx) => ({
  ...val,
...
export const quotationLineItemsSchema = z
  .array(quotationLineItemSchema)
  .min(1, "Add at least one line item");

const rawQuotationDiscountSchema = z.object({
  discountType: z.enum(["amount", "percentage"]),
  discountValue: z.union([z.string(), z.number()]),
  discountTarget: z.enum(DISCOUNT_TARGETS),
});

export const quotationDiscountSchema = rawQuotationDiscountSchema.transform((val, ctx) => ({
  ...val,
  discountValue: normalizeDiscountValue(val.discountType, val.discountValue, ctx),
}));
...
export const quotationListQuerySchema = z.object({
  ...
  tab: z.enum(["all", "draft", "open", "partial", "closed", "cancelled", "deleted"]).default("all"),
